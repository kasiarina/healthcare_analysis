...
for c in df.columns[1:-1]:  # Loop through the appropriate columns
    ...
    df[c] = df[c].apply(
        lambda x: str(x).split(" (")[0] if isinstance(x, str) else str(x)
    )
...
df_t = df[df["Have you had Covid`19 this year?"] == "Yes"].dropna(
    subset=["Maximum body temperature"]
)
sns.distplot(df_t["Maximum body temperature"], ax=axes[0])
df_t = df[df["Have you had Covid`19 this year?"] == "Maybe"].dropna(
    subset=["Maximum body temperature"]
)
